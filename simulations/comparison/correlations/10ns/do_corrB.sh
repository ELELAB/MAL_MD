...
pdb=../traj_files/${l}.frame0.gro
xtc=../traj_files/${l}.xtc
ndx=../index.ndx

echo -e "27\n0" | gmx_mpi trjconv -s $pdb -f $xtc -fit rot+trans -o traj_fitted.xtc -n $ndx 
echo "0" | gmx_mpi trjconv -s $pdb -f traj_fitted.xtc -o traj_cf_part.xtc -split $partsl -n $ndx
# 7: for each one:
trjsn=$(ls traj_cf_part*.xtc|wc -l)
for i in $(seq 0 $(($trjsn-2)) ); do
        xtcpart=traj_cf_part$i.xtc
	xtcpartfit=traj_cf_part_fit_all$i.xtc
        xtcpartfit2=traj_cf_part_fit$i.xtc
...
EOF
	...
	echo -e "27\n0" | gmx_mpi trjconv -f $xtcpart -s $pdb -fit rot+trans -o $xtcpartfit -n $ndx
        echo -e "27\n23" | gmx_mpi trjconv -f $xtcpart -s $pdb -fit rot+trans -o $xtcpartfit2 -n $ndx
        #echo "0" | g_covar -s $pdb -f $xtcpartfit -av average"$i".pdb -nofit
	echo "23" | gmx_mpi rmsf -s $pdb -f $xtcpartfit -ox average"$i".pdb -nofit -n $ndx; rm rmsf.xvg;
        ./add_segname -p average"$i".pdb -o average"$i".pdb -n B
        # 7c: convert xtc to dcd
        $wordom -imol average"$i".pdb -itrj $xtcpartfit2 -otrj $dcdpart -conv -nopbc
